...
export default function UploadFace() {
  ...
  const startCamera = async () => {
    ...
    try {
      await new Promise((resolve) => {
        const checkVideo = () => (videoRef.current ? resolve() : setTimeout(checkVideo, 50));
        checkVideo();
      });
  ...
  useEffect(() => () => stopAll(), []);
  const confirmCenteredManually = () => setFaceCentered(true);

  const captureSequenceFrames = async () => {
    if (!faceCentered) return alert("Centralize seu rosto antes de iniciar a captura.");
    ...
    for (let step = 0; step < movementSequence.length; step++) {
      ...
      for (let f = 0; f < framesPerMove; f++) {
        ...
        if (blob) allFrames.push(new File([blob], `frame_${movementSequence[step]}_${f}_${Date.now()}.jpg`, { type: "image/jpeg" }));
    ...
    try {
      ...
      setResponse(data);
      alert(`Sequência de movimentos capturada e enviada com sucesso! ${data.saved_faces} faces salvas.`);
    } catch (err) {
  ...
  return (
    <div className="upload-card">
      ...
      <div className="button-group" style={{ marginBottom: 12 }}>
        {!cameraActive && (
          <button className="start-button" onClick={startCamera} disabled={loading || !userId}>Ativar Câmera</button>
        )}
        ...
        {cameraActive && (
          <>
            <button
              ...
              onClick={captureSequenceFrames}
              disabled={loading || !faceCentered}
            >
              {loading ? "Capturando..." : faceCentered ? "Iniciar Captura de Movimentos" : "Aguardando centralização..."}
            </button>
            <button className="start-button" onClick={stopAll} style={{ background: "#f44336", marginLeft: 8 }}>Parar</button>
          </>
        )}
      </div>
      ...
      {cameraActive && (
        <div className="video-wrapper" style={{ position: "relative" }}>
          ...
          {/* Overlay de status facial */}
          <div className="arrow-overlay" style={{
            top: 12,
            left: "50%",
            transform: "translateX(-50%)",
            backgroundColor: faceDetected ? (faceCentered ? "rgba(46,170,65,0.85)" : "rgba(255,165,0,0.9)") : "rgba(220,20,60,0.9)"
          }}>
            {faceDetected ? (faceCentered ? "Rosto centralizado ✅" : "Ajuste seu rosto ao centro") : "Rosto não detectado"}
          </div>

          {/* Overlay de instrução de movimento */}
          {currentStep >= 0 && (
            <div className="movement-instruction" style={{
              position: "absolute",
              bottom: 20,
              left: "50%",
              transform: "translateX(-50%)",
              ...
              Olhe para: {movementSequence[currentStep]} ({currentFrame}/{framesPerMove})
            </div>
          )}
        </div>
      )}
